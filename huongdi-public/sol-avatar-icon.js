...
(function () {
  // ⭐ Skip khi embed trong iframe (VD: AI Studio tabs) — 2026-07-07
  ...
  function render() {
    ...
    if (state === 'free') {
      ...
      menu.innerHTML =
        ...
        '</div>' +
        '<a href="https://huongdi.sol.vn/tai-khoan/" class="sol-avatar-menu__item">' +
          '<span>👤</span> Tài khoản của tôi' +
        '</a>' +
        '<a href="https://huongdi.sol.vn/prompts/" class="sol-avatar-menu__item">' +
          '<span>🤖</span> 40 câu hỏi AI' +
        '</a>' +
        '<a href="https://huongdi.sol.vn/kham-pha-ban-than/" class="sol-avatar-menu__item">' +
          '<span>🧭</span> Vào Sol La Bàn' +
        '</a>' +
        '<button class="sol-avatar-menu__item sol-avatar-menu__item--logout" id="sol-avatar-logout">' +
          '<span>🚪</span> Đăng xuất' +
        '</button>';
    }

    // Tooltip (hiển khi hover)
    var tooltip = document.createElement('div');
    tooltip.className = 'sol-avatar-tooltip';
    ...
    if (state !== 'free') {
      var lo = document.getElementById('sol-avatar-logout');
