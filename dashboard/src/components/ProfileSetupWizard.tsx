// dashboard/src/components/ProfileSetupWizard.tsx
//
// 3-step wizard hỏi user về hồ sơ cai thuốc sau khi đăng nhập:
//   Bước 1: Tuổi + số năm đã hút
//   Bước 2: 1-3 lý do bỏ thuốc (preset + custom)
//   Bước 3: 1-3 trigger điển hình (preset + custom)
//
// Mirror với widget — cùng trường, cùng tone, cùng max-3 limit. Khác:
//   • dashboard render ở trung tâm màn hình (modal overlay)
//   • spacing rộng hơn, typography to hơn (web layout)
//   • có thể skip toàn bộ; sau khi dismiss 7 ngày không hỏi lại
...
export function ProfileSetupWizard({ onClose }: Props) {
  ...
  return (
    ...
    >
      <div className="sol-card-padded w-full max-w-lg max-h-[90vh] flex flex-col overflow-hidden">
        {/* Header */}
        <div className="flex items-start justify-between mb-4">
          <div>
            <div className="text-meta text-sol-ink-3">Bước {step}/3</div>
            <h2 className="text-h2 text-sol-ink">Hồ sơ cai thuốc của {pronouns}</h2>
          </div>
          <button
          ...
          {[1, 2, 3].map((s) => (
            <div
              ...
              className={clsx(
                ...
                s <= step ? 'bg-sol-green' : 'bg-sol-line',
              )}
            />
          ))}
        </div>
        ...
        <div className="flex-1 min-h-0 overflow-y-auto pr-1 space-y-5">
          ...
          {error && <div className="sol-alert-danger text-meta">{error}</div>}
        </div>

        {/* Footer */}
        <div className="flex items-center justify-between gap-2 mt-5 pt-4 border-t border-sol-line">
          <button
            onClick={dismiss}
...
function Step1({
  ...
  return (
    <div className="space-y-4">
      <div>
        <div className="text-h3 text-sol-ink mb-1">{Greet} ơi, mình hỏi 2 câu nhỏ</div>
        <p className="text-meta text-sol-ink-2">
          Để Sol đặt kỳ vọng phục hồi cho{' '}
    ...
    <div className="space-y-4">
      <div>
        <div className="text-h3 text-sol-ink mb-1">
          {Greet} bỏ thuốc <span className="text-sol-green-ink">vì điều gì?</span>
        </div>
        <p className="text-meta text-sol-ink-2">
      ...
      <div>
        <label className="sol-field-label">Hoặc viết lý do của riêng {pronouns}</label>
...
function Step3({
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        <label className="sol-field-label">Hoặc viết lúc khác {pronouns} hay thèm</label>
        <div className="flex gap-2">
          <input
            type="text"
            value={triggerCustom}
...
// ──────────────────────────────────────────────────────────────────────────
// Helper — quyết định có show wizard không
// ──────────────────────────────────────────────────────────────────────────
