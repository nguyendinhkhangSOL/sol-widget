...
const TABS: { id: Tab; emoji: string; label: string; color: string }[] = [
  { id: 'phase-1', emoji: '🌱', label: 'Nhận Diện', color: '#B25C2C' },
  { id: 'phase-2', emoji: '🔥', label: 'Kiểm Soát', color: '#B8860B' },
  { id: 'phase-3', emoji: '🚭', label: 'Làm Chủ',   color: '#3A7CA5' },
...
export function Analytics() {
  ...
  useEffect(() => {
    api.getJourneyDashboard()
      ...
      .catch((e) => setError(e instanceof ApiError ? `Lỗi kết nối (${e.status})` : 'Sol chưa tải được'));
  }, []);
    ...
    <div className="w-full max-w-[1100px] mx-auto p-4 lg:p-6 pb-24 lg:pb-8 space-y-6">
      <header className="px-1">
        ...
        <p className="text-body text-sol-ink-2 mt-1">
          Nhìn lại quy luật để hiểu mình hơn — không phải để tự trách.
        </p>
      </header>
...
function Phase1Analytics({ data }: { data: DashboardData }) {
  ...
  const TRIGGER_VN: Record<string, string> = {
    STRESS: 'Căng thẳng', EATING: 'Sau cơm', IDLE: 'Lúc rảnh', SOCIAL: 'Tụ tập / Nhậu', OTHER: 'Khác',
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Card title="🎯 Tình huống gây thèm phổ biến" accent="#B25C2C">
          {sortedTriggers.length === 0 ? (
            <p className="text-body text-sol-ink-3 italic">Chưa có ghi nhận. Bấm "+ Ghi điếu" và chọn tình huống để Sol bắt đầu học.</p>
          ) : (
        ...
        <Card title="📈 Số liệu tuần" accent="#B25C2C">
          ...
          <Stat2 label="Giờ thèm nhất hôm nay" value={data.today.peakHour !== null ? `${data.today.peakHour}h` : '—'} />
        </Card>
      </div>
      ...
      <div className="bg-sol-blue-soft/40 border border-sol-blue/20 rounded-2xl p-6">
        ...
        <p className="text-body-lg text-sol-ink leading-relaxed">
          Chặng Nhận Diện là tuần Sol đo nhịp của {data.user.pronouns}. Mục tiêu KHÔNG phải giảm —
          chỉ là quan sát để Sol thấy quy luật. Mỗi điếu ghi nhận là một dữ liệu Sol học cùng {data.user.pronouns}.
        </p>
      </div>
    </div>
  );
    ...
    <div className="space-y-6">
      <Card title="📉 Đường đi giảm dần" accent="#B8860B">
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          <Stat2 label="Mức nền (tuần Nhận Diện)" value={`${baseline}`} unit="điếu/ngày" />
          <Stat2
      ...
      <Card title="🎯 Đếm ngược Ngày Quyết Định" accent="#B8860B">
        {data.qDay.daysUntilQDay > 0 ? (
          <div>
            ...
            <p className="text-body text-sol-ink-2 mt-2 leading-relaxed">
              Còn {data.qDay.daysUntilQDay} ngày là Ngày Quyết Định — ngày {data.user.pronouns} cam kết bỏ hẳn.
              Tuần sau sẽ là tuần chuẩn bị tâm lý.
            </p>
          </div>
        ) : data.qDay.isQDay ? (
          <p className="text-h2 text-sol-orange-ink font-bold">🌅 Hôm nay là Ngày Quyết Định!</p>
        ) : (
          <p className="text-body text-sol-ink-3 italic">Ngày Quyết Định đã qua.</p>
        )}
      </Card>

      <Card title="✓ Hiệu quả Kế B" accent="#B8860B">
        ...
        <p className="text-meta text-sol-ink-3 mt-3 italic">
          Mỗi lần bỏ qua là một chiến thắng. Sol đếm cả những lần "thắng nhỏ" — vì chúng tạo nên thói quen mới.
        </p>
      </Card>
        ...
        {!breakdown || breakdown.days.length === 0 ? (
          ...
          <>
            <div className="text-meta text-sol-ink-2 mb-3">
              Mức nền: <strong>{breakdown.baseline} điếu/ngày × {fmt(breakdown.pricePerCig)}đ/điếu</strong>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 mt-4 pt-4 border-t border-sol-line">
              ...
              />
              <Stat2 label="Số ngày theo dõi" value={`${breakdown.days.length}`} unit="ngày" />
            </div>
          </>
        )}
      </Card>
      ...
      <Card title="🤝 So với Đội Sol" accent="#5C3A1E">
        {data.cohort.length === 0 ? (
          <p className="text-body text-sol-ink-3 italic">
            Chưa có đồng đội cùng lộ trình. Sol mong sớm có thêm người đi cùng {data.user.pronouns}.
          </p>
        ) : (
          <>
            <div className="text-body text-sol-ink mb-3">
              {data.cohort.length} đồng đội cùng tháng đặt Ngày Quyết Định với {data.user.pronouns}.
            </div>
  ...
  return (
    <div>
        ...
        {days.map((d) => {
          ...
          return (
            <div
              ...
              title={`Ngày ${d.day}: ${isPositive ? '+' : '−'}${fmt(Math.abs(d.cumulative))}đ`}
            />
          );
        })}
      </div>
      <div className="flex justify-between text-[11px] text-sol-ink-3 mt-1">
        <span>Ngày 1</span>
        <span>Ngày {Math.ceil(days.length / 2)}</span>
        <span>Ngày {days.length}</span>
      </div>
