<?php
/**
 * Plugin Name: Sol Landing Template (v2 — Thân · Tâm · Trí)
 * Description: Đăng ký page template "Sol Landing — Full HTML" cho các trang
 *              landing của Đi Cùng Sol. Template này KHÔNG render theme
 *              header/footer → full control HTML từ <html> tới </html>.
 *              v2 update: Header nav 3 trụ + Master Footer v3 (Brand + 5-col
 *              Nav + Safety+Disclaimer merged + Copyright).
 * Version:     2.0.0
 * Author:      Khang Sol
 * Updated:     2026-06-16
 *
 * Cài đặt:
 *   1. Backup file cũ:
 *      cp /var/www/sol.vn/wp-content/mu-plugins/sol-landing-template.php \
 *         /var/www/sol.vn/wp-content/mu-plugins/sol-landing-template.php.bak-2026-06-16
 *   2. Upload file này thay thế:
 *      → /var/www/sol.vn/wp-content/mu-plugins/sol-landing-template.php
 *   3. Mu-plugin tự active — KHÔNG cần Activate trong WP Admin
 *   4. Verify: mở landing page (vd /bo-thuoc-la/) → footer mới hiện
 *
 * Dùng:
 *   1. WP Admin → Pages → Add New / Edit page existing
 *   2. Sidebar phải → Page Attributes → Template
 *   3. Chọn "Sol Landing — Full HTML"
 *   4. Page render full screen với header sticky + Master Footer v3
 *   5. Content area max 1080px, hỗ trợ section break-out 100vw
 *
 * Thay đổi so với v1:
 *   ✅ Header nav: 3 trụ Thân-Tâm-Trí (Bothuocla / Tâm / Huongdi)
 *   ✅ Footer: Master v3 (4 zone, không phải mini block cũ)
 *   ✅ Schema Article + Person extended (sameAs, knowsAbout, image Yulong)
 *   ✅ JWT cross-domain mở rộng bothuocla + huongdi
 *   ✅ Triple YMYL disclaimer (Y khoa + Tinh thần + Tài chính)
 *
 *   ✓ Giữ nguyên: max-width 1080px, 5 responsive breakpoint, section break-out,
 *     table styling, wp-block-button, schema Article + Breadcrumb,
 *     special homepage title check, OG website type.
 */
...
<?php
// Title: homepage có format đặc biệt (Thân-Tâm-Trí)
if (is_front_page() || is_home()) {
    $page_title = 'SOL — Tái Thiết U45 · Thân · Tâm · Trí · Đi Cùng Sol';
} else {
...
$crumbs = [];
$position = 1;
$crumbs[] = ['@type' => 'ListItem', 'position' => $position++, 'name' => 'Trang chủ', 'item' => $site_url];
$ancestors = array_reverse(get_post_ancestors($post));
foreach ($ancestors as $ancestor_id) {
    $crumbs[] = ['@type' => 'ListItem', 'position' => $position++, 'name' => get_the_title($ancestor_id), 'item' => get_permalink($ancestor_id)];
}
...
/* ─── Landing content — 1080px max + section break-out ────────── */
.sol-landing-content {
...
/* Section break-out (100vw) */
.sol-landing-content > section[style*="100vw"] { width: auto; }
/* Force center inline blocks */
...
.sol-landing-content h1 { font-size: 32px; line-height: 1.2; font-weight: 700; margin: 24px 0 16px; color: var(--sol-earth); }
.sol-landing-content h2 { font-size: 24px; line-height: 1.3; font-weight: 700; margin: 32px 0 12px; color: var(--sol-earth); }
.sol-landing-content h3 { font-size: 19px; font-weight: 600; margin: 24px 0 8px; }
...
.sol-landing-content th, .sol-landing-content td {
  padding: 10px 12px; border: 1px solid var(--sol-line); text-align: left;
}
.sol-landing-content th { background: var(--sol-soft); font-weight: 600; }
.sol-landing-content a.wp-block-button__link,
.sol-landing-content .wp-block-button__link {
  display: inline-block; background: var(--sol-clay); color: white !important;
  padding: 14px 28px; border-radius: 8px;
  font-weight: 600; font-size: 16px; text-decoration: none; margin: 12px 0;
}
...
.sol-footer-tag {
  ...
  letter-spacing: 0.5px; padding: 2px 8px; border-radius: 4px; margin-right: 6px;
}
...
/* ─── Responsive 5 breakpoint (giữ từ v1) ─────────────────────── */
...
<!-- ─── Sol Widget — loaded by separate mu-plugin sol-widget-embed.php ─── -->

<!-- ─── JWT transfer cross-domain — sol.vn → bothuocla + huongdi ──────── -->
<script>
(function() {
