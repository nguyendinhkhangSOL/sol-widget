...
.sol-step.done { color:#0F172A; }
.sol-step.active { color:#D97706; font-weight:700; }
.sol-step-dot {
...
.sol-btn-view:hover { background:#FEF3C7; }
.sol-btn-reset {
  font-size:12px;
  color:#ef4444;
  background:transparent;
...
.sol-dna-fill.rank-1 { background:#0F172A; }
.sol-dna-fill.rank-2 { background:#F59E0B; }
...
// ─── Reset functions ──────────────────────────────────────────────────────────
function resetP1(){
  if(confirm('Xoá kết quả Khám phá bản thân và làm lại bài DNA Test?')){
    localStorage.removeItem('p1_result');
    window.location.href='/kham-pha-ban-than/';
  }
}
function resetP2(){
  if(confirm('Xoá kết quả Kiểm kê nguồn lực và khai báo lại?')){
    localStorage.removeItem('p2_result');
    window.location.href='/kiem-ke-nguon-luc/';
  }
}
function resetAll(){
  if(confirm('Xoá tất cả kết quả và bắt đầu lại từ đầu?')){
    localStorage.removeItem('p1_result');
    localStorage.removeItem('p2_result');
...
function injectProgressStrip(){
  ...
  const onP1 = /\/p1(\.html)?$/.test(path) || /\/kham-pha-ban-than\/?$/.test(path) || path === '/';
  const onP2 = /\/p2(\.html)?$/.test(path) || /\/kiem-ke-nguon-luc\/?$/.test(path);
  const onP3 = /\/p3/.test(path) || /\/la-ban-huong-di\/?$/.test(path);

  const stepData = [
    { n:1, label:'Khám phá bản thân', done:!!p1, active:onP1, href:'/kham-pha-ban-than/' },
    { n:2, label:'Kiểm kê nguồn lực', done:!!p2, active:onP2, href:'/kiem-ke-nguon-luc/' },
    { n:3, label:'La bàn hướng đi', done:false, active:onP3, href:'/la-ban-huong-di/' },
  ];
      ...
      <div class="sol-p1-card" style="border-style:dashed;background:#f9fafb;">
        <div class="sol-card-hd">
          <div class="sol-card-badge">
            <span class="ico">⭕</span>
            <div>
              <strong style="color:#6b7280">Khám phá bản thân (P1) chưa hoàn thành</strong>
              <span style="color:#9ca3af">Anh cần hoàn thành bước Khám phá trước</span>
            </div>
          </div>
          <a href="/kham-pha-ban-than/" class="sol-btn-view" style="background:#0F172A;color:#fff;border-color:#0F172A;">Bắt đầu Khám phá ngay →</a>
        </div>
      </div>`;
    return;
  }
    ...
    <div class="sol-p1-card">
      <div class="sol-card-hd">
        <div class="sol-card-badge">
          <span class="ico">✅</span>
          <div>
            <strong>Khám phá hoàn thành · Năng lực chính: ${primary}</strong>
            <span>${p1.title||'DNA Hướng Đi đã xác định'}</span>
          </div>
        </div>
        <div class="sol-card-actions">
          <a href="/kham-pha-ban-than/" class="sol-btn-view">Xem lại</a>
          <button class="sol-btn-reset" onclick="SOL.resetP1()">↺ Làm lại Khám phá</button>
        </div>
      </div>
      ...
      <div class="sol-p2-card" style="border-style:dashed;background:#f9fafb;">
        <div class="sol-card-hd">
          <div class="sol-card-badge">
            <span class="ico">⭕</span>
            <div>
              <strong style="color:#6b7280">Kiểm kê nguồn lực (P2) chưa hoàn thành</strong>
              <span style="color:#9ca3af">Cần khai báo nguồn lực để xem khớp chính xác</span>
            </div>
          </div>
          <a href="/kiem-ke-nguon-luc/" class="sol-btn-view" style="background:#f59e0b;color:#fff;border-color:#f59e0b;">Bắt đầu Kiểm kê ngay →</a>
        </div>
      </div>`;
    return;
  }
    ...
    <div class="sol-p2-card">
      <div class="sol-card-hd">
        <div class="sol-card-badge">
          <span class="ico">✅</span>
          <div>
            <strong>Kiểm kê hoàn thành · Mục tiêu: ${incomeText}/tháng</strong>
            <span>Nguồn lực mạnh nhất: ${resLabels[topResources[0].k]} (${topResources[0].v})</span>
          </div>
        </div>
        <div class="sol-card-actions">
          <a href="/kiem-ke-nguon-luc/" class="sol-btn-view">Xem lại</a>
          <button class="sol-btn-reset" onclick="SOL.resetP2()">↺ Làm lại Kiểm kê</button>
        </div>
      </div>
