/**
 * Sol API Sync — Non-breaking layer đồng bộ P1/P2/P3 với backend
 * ─────────────────────────────────────────────────────────────
 * Cách dùng:
 *   Add <script src="/js/sol-api-sync.js"></script> vào cuối buoc1/2/3.html
 *   → Script tự hook localStorage.setItem
 *   → Auto POST lên API khi user complete quiz
 *
 * Fallback: Nếu API fail, chỉ log warn, không phá luồng UI
 * Session: sessionId persist trong localStorage.sol_session_id
 *
...
(function() {
  ...
  function log(...args) {
    if (DEBUG || location.hostname === 'localhost') {
      console.log('[Sol API Sync]', ...args);
    }
  }

  function warn(...args) {
    console.warn('[Sol API Sync]', ...args);
  }
  ...
  async function syncP1(data) {
    try {
      ...
      // Robust score extraction: prefer normalized non-zero, fallback raw with inline normalize
      // Raw score range: 20-100 → normalized (raw - 20) / 80 * 100
      ...
      // Skip sync nếu user chưa complete quiz (tất cả = 0)
      const total = getScore('people') + getScore('expert') + getScore('builder') + getScore('independent');
      if (total === 0) {
        log('⚠ Skip P1 sync: quiz chưa complete (tất cả scores = 0)');
        return;
      }
      ...
      warn('P1 sync fail (silent):', e.message);
    }
  }
  ...
  async function syncP2(data) {
    try {
      ...
      // Skip nếu tất cả = 0 (quiz chưa complete)
      ...
      // Map incIdeal number → incomeGoal label
      ...
      warn('P2 sync fail (silent):', e.message);
    }
  }
  ...
  // ─── Load Directions from API (for buoc3.html) ──────────────
  ...
  // ─── Save Direction to Sổ Hành Trình (P3 button) ────────────
  ...
  // Ví dụ user đã có localStorage.p1_result từ trước → sync ngay
  ...
  // ─── Page View Tracking ─────────────────────────────────────
  // Note: EventType enum chỉ có P1_START (không có P2_START), P3_VIEW
  ...
  // P2 không có START event — chỉ có COMPLETE khi user finish

  if (pageEvent) {
    // Delay 500ms để không block page load
    setTimeout(() => trackEvent(pageEvent, {}), 500);
  }

  log('Initialized. Session:', getSessionId(), 'API:', API_BASE);
})();
