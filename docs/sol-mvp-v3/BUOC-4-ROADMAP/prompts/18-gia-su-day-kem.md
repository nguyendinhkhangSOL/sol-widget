...
**Sort order**: 3
**Status**: ⬜ TODO
**Direction ID**: `637a78f4-b431-4d9a-ae31-51c196ce14b5`
...
6. Copy JSON output → save thành `roadmap-gia-su-day-kem.json` trong folder `generated/`
...
**Description**:
Dạy kèm các môn học, ngoại ngữ, hoặc kỹ năng nghề nghiệp.

**Vì sao phù hợp 40-60**:
Entry barrier thấp nhất, thu nhập ngay. Phù hợp người có kiến thức nền tốt.

**Rào cản đặc thù**:
[
  "Thu nhập có giới hạn (đổi thời gian lấy tiền)",
  "Khó scale"
]

**DNA fit scores (0-100)**:
- People (thích tiếp xúc): 80
- Expert (chuyên môn sâu): 75
- Builder (thích tự xây): 25
...
1. `version`: "1.0"
2. `direction_id`: "637a78f4-b431-4d9a-ae31-51c196ce14b5"
3. `direction_name`: "Gia Sư / Dạy Kèm"
4. `total_weeks`: 12
5. `total_actions`: 40-50 (tổng actions trong 12 tuần)
6. `phases`: 3 giai đoạn `[Định vị (T1-4), Momentum (T5-8), Khách #1 (T9-12)]` — MỖI phase có `goal` cụ thể cho mô hình **Gia Sư / Dạy Kèm**
...
- **Considering DNA scores cho mô hình này**:
  ...
  - vp_expert 75/100 → nhiều content/thought leadership actions
