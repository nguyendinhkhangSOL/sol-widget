# 05. Dịch Vụ Vận Chuyển / Giao Hàng

**Slug**: `van-chuyen-giao-hang`
**Category**: DAILY
**Sort order**: 1
**Status**: ⬜ TODO
**Direction ID**: `3df3e158-5564-48d2-9dbc-a25216c1cac4`
...
6. Copy JSON output → save thành `roadmap-van-chuyen-giao-hang.json` trong folder `generated/`
...
**Tên**: Dịch Vụ Vận Chuyển / Giao Hàng
**Slug**: van-chuyen-giao-hang
**Tagline**: Vận chuyển hàng hóa, giao hàng chặng cuối
**Category**: DAILY

**Description**:
Cung cấp dịch vụ vận chuyển, giao hàng cho cá nhân và doanh nghiệp địa phương.

**Vì sao phù hợp 40-60**:
Phù hợp người có phương tiện, muốn thu nhập ngay, thích vận động.

**Rào cản đặc thù**:
[
  "Thu nhập giới hạn theo thời gian làm việc",
  "Hao mòn phương tiện"
]

**DNA fit scores (0-100)**:
- People (thích tiếp xúc): 55
- Expert (chuyên môn sâu): 25
- Builder (thích tự xây): 45
...
- Network (quan hệ): 50
- Risk (rủi ro): 30
- Energy (năng lượng): 75

**Bonus metrics (0-100)**:
- Income speed (tốc độ có thu nhập): 85
- Income potential (tiềm năng thu nhập): 45
- Scalability (khả năng scale): 50
- AI leverage (leverage được AI): 40
...
1. `version`: "1.0"
2. `direction_id`: "3df3e158-5564-48d2-9dbc-a25216c1cac4"
3. `direction_name`: "Dịch Vụ Vận Chuyển / Giao Hàng"
4. `total_weeks`: 12
5. `total_actions`: 40-50 (tổng actions trong 12 tuần)
6. `phases`: 3 giai đoạn `[Định vị (T1-4), Momentum (T5-8), Khách #1 (T9-12)]` — MỖI phase có `goal` cụ thể cho mô hình **Dịch Vụ Vận Chuyển / Giao Hàng**
...
- **Considering DNA scores cho mô hình này**:
  - vp_people 55/100 → ít outreach actions
  - vp_expert 25/100 → ít content/thought leadership actions
- **Considering resources**:
  - vr_capital 40/100 → capital thấp, focus effort thay vì vốn
  - vr_tech 40/100 → tech không phải bottleneck
