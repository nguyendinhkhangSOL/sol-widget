# 13. Giảng Dạy Online (Khóa Học)

**Slug**: `giang-day-online`
**Category**: DAO_TAO
**Sort order**: 2
**Status**: ⬜ TODO
**Direction ID**: `9789abba-2cc8-4d64-9e39-2a579fd6c799`
...
6. Copy JSON output → save thành `roadmap-giang-day-online.json` trong folder `generated/`
...
**Tên**: Giảng Dạy Online (Khóa Học)
**Slug**: giang-day-online
**Tagline**: Tạo và bán khóa học trực tuyến
**Category**: DAO_TAO

**Description**:
Thiết kế và bán khóa học online trên Udemy, Kyna, hoặc website riêng.

**Vì sao phù hợp 40-60**:
Phù hợp người muốn thu nhập thụ động dài hạn, sẵn sàng đầu tư thời gian ban đầu.

**Rào cản đặc thù**:
[
  "Cần thời gian dài để xây audience",
  "Chất lượng video & âm thanh quan trọng"
]

**DNA fit scores (0-100)**:
- People (thích tiếp xúc): 65
- Expert (chuyên môn sâu): 85
- Builder (thích tự xây): 70
- Independent (tự chủ): 80
...
- Network (quan hệ): 55
- Risk (rủi ro): 35
- Energy (năng lượng): 60

**Bonus metrics (0-100)**:
- Income speed (tốc độ có thu nhập): 30
- Income potential (tiềm năng thu nhập): 90
- Scalability (khả năng scale): 90
- AI leverage (leverage được AI): 75
...
1. `version`: "1.0"
2. `direction_id`: "9789abba-2cc8-4d64-9e39-2a579fd6c799"
3. `direction_name`: "Giảng Dạy Online (Khóa Học)"
4. `total_weeks`: 12
5. `total_actions`: 40-50 (tổng actions trong 12 tuần)
6. `phases`: 3 giai đoạn `[Định vị (T1-4), Momentum (T5-8), Khách #1 (T9-12)]` — MỖI phase có `goal` cụ thể cho mô hình **Giảng Dạy Online (Khóa Học)**
...
- **Considering DNA scores cho mô hình này**:
  - vp_people 65/100 → nhiều outreach actions
  - vp_expert 85/100 → nhiều content/thought leadership actions
- **Considering resources**:
  - vr_capital 30/100 → capital thấp, focus effort thay vì vốn
  - vr_tech 65/100 → phase 1 phải học tech tools
