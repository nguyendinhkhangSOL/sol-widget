/**
 * Fagerström Test for Nicotine Dependence (FTND) — Sol Widget v0.2
 *
 * Map FTND score → Cohort LIGHT/MODERATE/HEAVY (theo /gia/ page)
 *
 * Pricing model:
...
export interface CohortPlan {
  id: Cohort;
  name: string;                  // "Lộ trình 35 ngày"
  audienceLabel: string;         // "Hút dưới 10 điếu/ngày"
  freeDays: number;              // 7
  paidDays: number;              // 28 / 45 / 58
  totalDays: number;             // 35 / 52 / 65
  dailyRate: number;             // 5000 (đồng)
  totalPrice: number;            // 140000 / 225000 / 290000
  weeklyRate: number;            // 35000 (alternative pay-as-you-go)
...
/**
 * 6 câu hỏi FTND chuẩn (Heatherton 1991) — dịch tiếng Việt cho Sol audience
 */
...
/**
 * 3 Cohort Plans (theo sol.vn/gia/)
 */
...
/**
 * Get full result với plan + meaning
 */
export function getFtndResult(answers: FtndAnswer[]): FtndResult {
  ...
  return {
    ...
    cohort,
    plan,
    scoreRange: scoreRanges[cohort],
    whatItMeans: whatItMeans[cohort]
  };
}
