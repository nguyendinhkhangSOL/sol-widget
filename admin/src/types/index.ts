...
export interface TierCatalogItem {
  id: UserTier;
  label: string;
  /** Sol v3: emoji của chặng (🌱 🟡 🔴 🌟) */
  emoji?: string;
  /** Sol v3: tagline ngắn mô tả mục tiêu chặng */
  tagline?: string;
  priceVnd: number;
  durationDays: number | null;
  refundable?: boolean;
  /** Sol v3: 'conditional' (Kiểm Soát Day 21) | 'prorated' (Làm Chủ Day 14+) */
  refundType?: 'conditional' | 'prorated';
  refundFromDay?: number;
  /** Sol v3: Day 22 = Q-Day Ceremony cho Làm Chủ */
  qDayDay?: number;
  /** Sol v3: tier KHUYẾN NGHỊ — hiển thị badge */
  recommended?: boolean;
  /** Sol v3: ALUMNI miễn phí mãi mãi */
  forever?: boolean;
  bullets: string[];
  callout?: string;
}
...
export interface TierCatalog {
  ...
  /** Sol v3: schedule overview cho UI */
  schedule?: {
    totalDaysSolActive: number;     // 51
    qDayDay: number;                // 22
    graduationDay: number;          // 52
    totalPaidVnd: number;           // 298.000
  };
  note: {
    freeDailyQuota: number;
    /** Sol v3 renamed: maintenanceDailyQuota → alumniDailyQuota (vẫn giữ backward compat) */
    maintenanceDailyQuota?: number;
    alumniDailyQuota?: number;
    /** Sol v3: deprecate maintenance window — luôn = 0 */
