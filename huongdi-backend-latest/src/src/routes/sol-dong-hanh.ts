// ═══════════════════════════════════════════════════════════════
// /api/sol-dong-hanh/* — Multi-provider v3
...
const PROVIDER: string | null = GEMINI_KEY ? 'gemini'
  : OPENAI_KEY ? 'openai'
...
const MODEL_CONFIG: Record<string, Record<string, { model: string; input: number; output: number }>> = {
  gemini: {
    ACTIVE: { model: 'gemini-2.5-flash', input: 0.075, output: 0.30 },
    FOUNDER: { model: 'gemini-2.5-flash', input: 1.25, output: 5.00 },
  },
  openai: {
...
const TIER_QUOTA: Record<string, number> = { ACTIVE: 30, FOUNDER: 500 };
  ...
  try {
    ...
    if (!PROVIDER) {
      return res.status(503).json({
        ...
        message: 'AI provider chưa cấu hình.',
      });
    }
    ...
    return res.json({
      ...
      provider: PROVIDER, model: modelInfo.model,
      quota: {
...
router.post('/chat', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    if (tier !== 'ACTIVE' && tier !== 'FOUNDER') {
      return res.status(403).json({ success: false, message: 'Chỉ Active/Founder', upgradeUrl: '/thanh-toan/' });
    }
    if (!PROVIDER) return res.status(503).json({ success: false, message: 'AI chưa cấu hình' });

    const monthKey = getCurrentMonthKey();
    const quota = await getOrInitQuota(userId, monthKey, tier);
    if (quota.messagesUsed >= quota.quotaLimit) {
      return res.status(429).json({ success: false, message: `Hết ${quota.quotaLimit} msg`, quota });
    }
    ...
    if (conversationId) {
      ...
      conversation = await (prisma as any).solChatConversation.create({
        data: {
          userId, directionId: context.direction?.id || null, phase: context.phase,
    ...
    const modelInfo = MODEL_CONFIG[PROVIDER][tier];
    let assistantMessage = '', tokensIn = 0, tokensOut = 0;
    ...
    if (PROVIDER === 'gemini' && gemini) {
      ...
      const response = await anthropic.messages.create({
        model: modelInfo.model, max_tokens: 1024, system: systemPrompt,
        messages: [
          ...history.map((m: any) => ({ role: m.role, content: m.content })),
          { role: 'user', content: message },
        ] as any,
    ...
    return res.json({
      ...
      usage: { tokensIn, tokensOut, costUsd: costUsd.toFixed(4), model: modelInfo.model, provider: PROVIDER },
      quota: {
        used: quota.messagesUsed + 1, limit: quota.quotaLimit,
        remaining: quota.quotaLimit - quota.messagesUsed - 1,
      },
    });
  } catch (err: any) {
    console.error('[POST /chat]', err);
    return res.status(500).json({ success: false, message: 'Lỗi AI: ' + (err.message || 'unknown') });
  }
});
...
async function loadUserContext(userId: string) {
  const user = await prisma.user.findUnique({
    where: { id: userId }, select: { displayName: true, tier: true } as any,
  });
